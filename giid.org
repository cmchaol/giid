...
#+RESULTS:
-rwxr-xr-x 1 c5766 c5766 1.1K Sep 11 10:08 build.sh
#!/bin/bash
...
docker run -v $d:/tmp -it  c5766/giid:x21

echo FEATURES="buildpkg" >> /etc/portage/make.conf

echo "app-editors/emacs xft X jpeg png svg tiff gif imagemagick" > /etc/portage/package.use/emacs

echo "app-emacs/emacs-common-gentoo X " > /etc/portage/package.use/emacs-common-gentoo
...
docker commit  22f4efbcf09c        c5766/giid:e12


docker login


docker push c5766/giid:e12
...
docker run -v $d:/tmp -it  c5766/giid:x18
...
docker commit      4a07118f30d3        c5766/giid:x19


docker login


docker push c5766/giid:x19
...
|-----+--------+-----------------------------|
| t12 | g12    | disk full                   |
| t13 |        | 0.5G, excludce src, portage |
| t14 |        |                             |



f=https://transfer.sh/94cHF/q12_20170913-2.tar.xz
...
lftp --parallel=10 $f  # fails

lftp -P 10 $f  # fails



lftp -c "set https:ssl-allow no; pget -n 10 $f"

lftp -c "set ssl:verify-certificate no; pget -n 10 $f"
...
https://transfer.sh/jFpCz/q12_20170912.tar.xz
...
1881.2 MiB / 4338.5 MiB = 0.434, 3.0 MiB/s, 24:00
...
|       |      |                              | kernel before xorg             |
|-------+------+------------------------------+--------------------------------|
| x16   | b35  | prepare for llvm             | script error                   |
| x17   | b36  | prepare for llvm             | b36 time up                    |
| x18   | b37  | prepare for llvm             |                                |
|       |      |                              |                                |
| x19   | x18  | llvm @ dply                  | succeeds, 110 min              |
| x20   | x19  | xorg-drivers                 |                                |
| x21   | x20  |                              |                                |
...
#+RESULTS:
-rwxr-xr-x 1 user1 user1 489 Sep  9 11:26 build.sh
#!/bin/bash
...
docker run -v $d:/tmp -it  c5766/giid:b35
...
| b34 |         | kernel first          |                              |
|     |         |                       |                              |
| b35 | gentoo  | b34 + binary package  | 100 min                      |
| b36 |         |                       | adjust USE flag manipulation |
|     |         |                       | time up                      |
|     |         |                       |                              |
| b37 |         |                       | remove 2 packages            |
...
Installing (39 of 39) dev-vcs/git-2.13.5::gentoo
...
| tag | parent | results | errors                    |
|-----+--------+---------+---------------------------|
| q11 | d11    |         | USE changes are necessary |
| q2  |        |         |                           |
...
docker run -v $d:/tmp -it  c5766/giid:g11

echo FEATURES="buildpkg" >> /etc/portage/make.conf

emerge \
       app-emulation/docker -pv
...
docker commit 2e4b4dcc2bd4        c5766/giid:d11


docker login


docker push c5766/giid:d11
...
The following USE changes are necessary to proceed:
 (see "package.use" in the portage(5) man page for more details)
# required by app-emulation/qemu-2.9.0-r56::gentoo[-static,sdl,-sdl2]
...
Installing (15 of 15) app-emulation/docker-17.03.1::gentoo
 * 
...
>>> Recording app-emulation/docker in "world" favorites file...
...
| tag | parent | results  |                   |
|-----+--------+----------+-------------------|
| d11 | g11    | succeeds | ker448-20170912-2 |
|     |        |          |                   |
...
lftp -u user,pwd \
-e "set sftp:connect-program 'ssh -a -x -T -c arcfour -o Compression=no'; mirror -v -c --loop --use-pget-n=3 -P 2 /remote/dir/ /local/dir/; quit" \
sftp://remotehost:22
...
s=" ssh -J ndmc@$h1:443 root@$h2 -p 443 "
...
lftp \
-e "set sftp:connect-program 'ssh -a -x -T -J ndmc@$h1:443'" \
sftp://root:DUMMY@$h2:443



pget -n 10 /tmp/q12_20170913-2.tar.xz
