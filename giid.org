...
mount -o ro \$(findfs UUID=\$u4) /tmp/sqfsdevice


mkdir /tmp/root-ro
f=/tmp/sqfsdevice/sqfs
mount -o loop \$f /tmp/root-ro

mkdir /tmp/upper /tmp/work /tmp/root-rw

mount -t overlay overlay -olowerdir=/tmp/root-ro,upperdir=/tmp/upper,workdir=/tmp/work /tmp/root-rw

mkdir -p /tmp/root-rw/tmp/upper /tmp/root-rw/tmp/work /tmp/root-rw/tmp/root-rw

mount --bind /tmp/upper /tmp/root-rw/tmp/upper
mount --bind /tmp/work /tmp/root-rw/tmp/work
mount --bind /tmp/root-rw /tmp/root-rw/tmp/root-rw
...
# Boot the real thing.
exec switch_root /tmp/root-rw /sbin/init
EOF
